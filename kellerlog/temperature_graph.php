...
while (($col = fgetcsv($file, 100, ",")) !== FALSE) {
	array_push($data, array($col[0],$col[1]));
}
fclose($file);
...
for (var i = -20; i <= 40; i+=5) {
	let y = mapfloat(i, -20, 40, canvas.height, 0);
    ctx.fillRect(0,y, canvas.width,1);
    ctx.fillText(i + "°C", 3, y-10);
}
...
for (var x = 1; x < data.length; x+=1) {
	...
	let y = mapfloat(data[x][1], -20, 40, canvas.height, 0); //Draw data point
    if (x == 1) {
        ctx.moveTo(x,y);
    }
    else {
